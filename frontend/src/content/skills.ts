import type { Skill } from '@/types/content';

/**
 * `proficiency` is new in this rebuild — it drives inventory item rarity in the
 * pixel room. The values below are a first pass; they are the one field here
 * that was not carried over from the old site, so they are worth a review.
 *
 * The old `tailwind-icon` class was not a real Font Awesome class and rendered
 * an empty box; it is `fa-wind` now.
 */
export const skills: Skill[] = [
  {
    id: 1,
    name: 'React',
    icon: 'fab fa-react',
    description:
      'Building interactive UIs with modern React features including hooks, context API, and custom components. Expert in React ecosystem.',
    category: 'frontend',
    proficiency: 5,
    itemSprite: 'item-react',
    sortOrder: 1,
  },
  {
    id: 2,
    name: 'JavaScript',
    icon: 'fab fa-js-square',
    description:
      'Creating dynamic web experiences with ES6+ features, asynchronous programming, and modern JavaScript patterns and best practices.',
    category: 'frontend',
    proficiency: 5,
    itemSprite: 'item-javascript',
    sortOrder: 2,
  },
  {
    id: 3,
    name: 'HTML5 & CSS3',
    icon: 'fab fa-html5',
    description:
      'Crafting semantic markup and responsive designs with modern CSS techniques including Grid, Flexbox, and CSS animations.',
    category: 'frontend',
    proficiency: 5,
    itemSprite: 'item-html',
    sortOrder: 3,
  },
  {
    id: 4,
    name: 'Tailwind CSS',
    icon: 'fas fa-wind',
    description:
      'Creating beautiful, responsive designs with utility-first CSS framework. Experienced in custom configuration and component design.',
    category: 'frontend',
    proficiency: 5,
    itemSprite: 'item-tailwind',
    sortOrder: 4,
  },
  {
    id: 5,
    name: 'UI/UX Design',
    icon: 'fas fa-palette',
    description:
      'Designing intuitive interfaces focused on accessibility and user experience. Skilled in wireframing, prototyping, and user testing.',
    category: 'frontend',
    proficiency: 3,
    itemSprite: 'item-palette',
    sortOrder: 5,
  },
  {
    id: 6,
    name: 'Git & GitHub',
    icon: 'fab fa-git-alt',
    description:
      'Version control and collaborative development with Git and GitHub. Experienced with branching strategies, pull requests, and team workflows.',
    category: 'development',
    proficiency: 4,
    itemSprite: 'item-git',
    sortOrder: 6,
  },
  {
    id: 7,
    name: 'Node.js',
    icon: 'fab fa-node-js',
    description:
      'Building scalable server-side applications with Node.js. Proficient in Express.js for RESTful APIs and backend services.',
    category: 'backend',
    proficiency: 4,
    itemSprite: 'item-node',
    sortOrder: 7,
  },
  {
    id: 8,
    name: 'JWT',
    icon: 'fas fa-key',
    description:
      'Implementing secure token-based authentication with JWT. Experienced in auth flows, token refresh, and secure API access control.',
    category: 'authentication',
    proficiency: 4,
    itemSprite: 'item-key',
    sortOrder: 8,
  },
  {
    id: 9,
    name: 'bcrypt',
    icon: 'fas fa-lock',
    description:
      'Securing user credentials using bcrypt hashing. Skilled in password storage, verification, and best practices for authentication security.',
    category: 'authentication',
    proficiency: 4,
    itemSprite: 'item-lock',
    sortOrder: 9,
  },
  {
    id: 10,
    name: 'FastAPI',
    icon: 'fab fa-python',
    description:
      'Developing high-performance Python APIs with FastAPI. Skilled in async endpoints, dependency injection, and automatic OpenAPI documentation.',
    category: 'backend',
    proficiency: 4,
    itemSprite: 'item-fastapi',
    sortOrder: 10,
  },
  {
    id: 11,
    name: 'SQLAlchemy',
    icon: 'fab fa-python',
    description:
      'Designing ORM-driven Python data models with SQLAlchemy. Experienced in database schema design, relationships, and query optimization.',
    category: 'backend',
    proficiency: 3,
    itemSprite: 'item-sqlalchemy',
    sortOrder: 11,
  },
  {
    id: 12,
    name: 'Sequelize ORM',
    icon: 'fas fa-database',
    description:
      'Expertise in database modeling and management using Sequelize ORM with Node.js. Proficient in relationships, migrations, and queries.',
    category: 'backend',
    proficiency: 4,
    itemSprite: 'item-sequelize',
    sortOrder: 12,
  },
  {
    id: 13,
    name: 'PostgreSQL',
    icon: 'fas fa-database',
    description:
      'Advanced database design and management with PostgreSQL. Experienced in complex queries, optimizations, and data modeling.',
    category: 'database',
    proficiency: 4,
    itemSprite: 'item-postgres',
    sortOrder: 13,
  },
];

export const skillCategories = [
  { key: 'all', label: 'All' },
  { key: 'frontend', label: 'Frontend' },
  { key: 'backend', label: 'Backend' },
  { key: 'database', label: 'Database' },
  { key: 'development', label: 'Development' },
  { key: 'authentication', label: 'Authentication' },
] as const;

export type SkillFilter = (typeof skillCategories)[number]['key'];
